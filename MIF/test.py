...
if __name__ == "__main__":
    ...
    size = (256, 256)
    num_iter = 1
    checkpoint_path = "filesU/cp182.pth"

    """ Directories """
    create_dir("resultsU182and")
    ...
    """ Testing """
    prev_masks,prev_ft = init_mask(test_x, size)
    save_data = []
    file = open("filest520/test_results18.csv", "w")
    ...
    for iter in range(num_iter):
        ...
        for i, (x, y) in tqdm(enumerate(zip(test_x, test_y)), total=len(test_x)):
            ...
            pft = prev_ft[i]
            pft = " ".join(str(d) for d in pft)
            pft = str(pft)
            pft = rle_decode(pft, size)
            pft = np.expand_dims(pft, axis=0)
            pft = np.expand_dims(pft, axis=0)
            pft = pft.astype(np.float32)
            if iter == 0:
                pft = np.transpose(pft, (0, 1, 3, 2))
            pft =  torch.from_numpy(pft)
            pft = pft.to(device)

            with torch.no_grad():
                """ FPS Calculation """
                start_time = time.time()
                pred_y = torch.sigmoid(model([image, pmask,pft]))
    ...
    #     cv2.imwrite(f"resultsU182and/{name}.png", cat_images)
